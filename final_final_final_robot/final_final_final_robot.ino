...
void loop() {
  ...
  if (start == true) {
    ...
    if ((now - last_loop_time >= loop_interval) || first == true) {  // keep loop timing consistent
      ...
      if ((sensorValues[0] >= black_line || leftEdgeCount >= turnThreshold) && (sensorValues[5] >= black_line || rightEdgeCount >= turnThreshold)) {
        // Intersection
        //start = false;
        leftEdgeCount = 0;
        rightEdgeCount = 0;
